          ...
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              <?php foreach($result as $key=>$iteresult): 
                ?>
              <tr>
                <td rowspan="<?= $iteresult->colspan+1?>"><?= $key+1?></td>
                <td rowspan="<?= $iteresult->colspan+1?>"><?= $iteresult->distrik?></td>
              </tr>
              <?php 
              foreach($iteresult->kategori as $keykategori => $itemkategori):?>
                <tr>
                  <td rowspan="<?=  $itemkategori['colspan']+1?>"><?= $itemkategori['kategori']?></td>   
                  <?php if(count($itemkategori['usaha'])==0):?>
                    <td></td>
                    <td></td>
                  <?php endif;?>
                </tr>
              <?php 
              foreach($itemkategori['usaha'] as $keyusaha => $itemusaha): 
              ?>
                <tr>
                  <td><?= $itemusaha['nama']?></td>                  
                  <td><?= $itemusaha['alamat']?></td>                  
                </tr>  
              <?php 
                  endforeach;
                endforeach;
